...
int main(int argc, char *argv[]) {
	...
	msgflg = (IPC_NOWAIT | MSG_COPY);
	len	= sizeof(data_st) - sizeof(long);
	while (1) {
			nbytes = msgrcv(mid, (struct data *) &data_st, len, msgtyp, msgflg);
			if (nbytes == -1) {
				printf("msgrcv failed (%m)\n");
				exit(EXIT_FAILURE);
			}
			printf("type: %ld\t%d: %s\n",data_st.mtype, data_st.from, data_st.message);
	}
